 ...
 */
export function parseTxt(text: string): Seal[] {
  const lines = text
    .split('\n')
    .map(line => line.trim())
 ...
 */
export function isValidCode(codigo: string): boolean {
 ...
 */
export function validateSeals(seals: Seal[]): { valid: Seal[]; invalid: Seal[] } {
  const valid: Seal[] = [];
  const invalid: Seal[] = [];
